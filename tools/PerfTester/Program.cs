            ...
            for (var repeat = 0; repeat < _repeat; repeat++)
            {
                BatchExtensions.ExecuteInParallelBatches(resourceIds, _threads, resourceIdsPerCall, (thread, resourceIds) =>
                {
                    ...
                    if (_callType == "GetAsync")
                    {
                        ...
                        var first = _store.GetAsync(new[] { new ResourceDateLocationKey(typeId, id, 0, null, 0, 0) }, (s) => "xyz", (i) => typeId.ToString(), true, CancellationToken.None).Result.FirstOrDefault();
                        if (first == null)
                        {
                            Interlocked.Increment(ref errors);
                        }
