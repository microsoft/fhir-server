        ...
        private static string PutResource(string jsonString, string resourceType, string resourceId)
        {
            ...
            do
            {
                ...
                try
                {
                    ...
                    if (response.StatusCode == HttpStatusCode.InternalServerError && _callType == "HttpCreate")
                    {
                        specialError = true;
                    }
                ...
                {
                    Thread.Sleep(delay);
                    delay = (int)(delay * (2 + (RandomNumberGenerator.GetInt32(1000) / 1000.0)));
                }
            }
            while ((bad && retries < maxRetries) || specialError);
            if (bad)
            {
