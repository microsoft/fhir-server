...
namespace Microsoft.Health.Internal.Fhir.BlobRewriter
{
    internal static class Program
    {
        ...
        private static BlobContainerClient GetContainer(string storageConnectionString, string storageUri, string storageUAMI, string storageContainerName)
        {
            try
            {
                var blobServiceClient = string.IsNullOrEmpty(storageUri) ? new BlobServiceClient(storageConnectionString) : new BlobServiceClient(new Uri(storageUri), string.IsNullOrEmpty(storageUAMI) ? new InteractiveBrowserCredential() : new ManagedIdentityCredential(ManagedIdentityId.FromUserAssignedClientId(storageUAMI)));
