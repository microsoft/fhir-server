...
namespace IndexRebuilder
{
    internal class StoreUtils
    {
        ...
        internal static Task StartTask(Action action, bool longRunning = true)
        {
        ...
        internal static void ExecuteInParallelBatches<T>(IEnumerable<T> objects, int threads, int batchSize, Action<int, Tuple<int, IList<T>>> action, int? queueCapacity = null, CancelRequest cancel = null)
        {
            if (cancel == null)
            {
