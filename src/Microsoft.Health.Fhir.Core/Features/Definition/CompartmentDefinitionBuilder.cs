...
namespace Microsoft.Health.Fhir.Core.Features.Definition
{
    internal class CompartmentDefinitionBuilder
    {
        ...
        private static (Dictionary<CompartmentType, CompartmentDefinition>, Dictionary<ResourceType, Dictionary<CompartmentType, HashSet<string>>>) Build(Bundle bundle)
        {
            Dictionary<CompartmentType, CompartmentDefinition> compartmentTypeDictionary = ValidateAndGetCompartmentDict(bundle);

            Dictionary<ResourceType, Dictionary<CompartmentType, HashSet<string>>> compartmentLookup = BuildResourceTypeLookup(compartmentTypeDictionary.Values);
        ...
        private static Dictionary<ResourceType, Dictionary<CompartmentType, HashSet<string>>> BuildResourceTypeLookup(ICollection<CompartmentDefinition> compartmentDefinitions)
        {
            ...
            foreach (CompartmentDefinition compartment in compartmentDefinitions)
            {
                foreach (ResourceComponent resource in compartment.Resource)
                {
                    ...
                    resourceTypeDict[compartment.Code.Value] = resource.Param?.ToHashSet();
                }
            }
