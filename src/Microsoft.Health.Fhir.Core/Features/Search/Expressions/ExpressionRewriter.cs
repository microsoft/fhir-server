...
namespace Microsoft.Health.Fhir.Core.Features.Search.Expressions
{
    ...
    public abstract class ExpressionRewriter<TContext> : IExpressionVisitor<TContext, Expression>
    {
        ...
        public Expression VisitNotReferenced(NotReferencedExpression expression, TContext context)
        {
            return expression;
        }

        public virtual Expression VisitTrustedResourceIdList(TrustedResourceIdListExpression expression, TContext context)
        {
            return expression;
        }
...
#pragma warning restore CA1002 // Do not expose generic lists
            ...
            where TDestination : TSource
        {
            if (destination == null)
            {
                ...
                for (int j = 0; j < count; j++)
                {
                    if (source[j] is TrustedResourceIdListExpression)
                    {
