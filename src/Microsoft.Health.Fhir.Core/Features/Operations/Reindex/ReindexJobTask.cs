...
namespace Microsoft.Health.Fhir.Core.Features.Operations.Reindex
{
    public sealed class ReindexJobTask : IReindexJobTask, IDisposable
    {
        ...
        private async Task<bool> TryPopulateNewJobFields(CancellationToken cancellationToken)
        {
            ...
            var possibleNotYetIndexedParams = _searchParameterDefinitionManager.AllSearchParameters.Where(sp => validStatus.Contains(searchParamStatusCollection.First(p => p.Uri == sp.Url).Status) || sp.IsSearchable == true || sp.SortStatus == SortParameterStatus.Supported);
