...
namespace Microsoft.Health.Fhir.SqlServer.UnitTests.Features.Search.Expressions
{
    public class IncludeRewriterTests
    {
        ...
        [Fact]
        public void GivenASqlRootExpressionWithOrphanIncludeParameter_WhenVisitedByIncludeRewriter_OrphanIncludeIsRemoved()
        {
            // Remove the orphan expressions from this query (i.e., include expressions without an included result set)
            // [base]/MedicationDispense?_include=MedicationDispense:prescription&_include=Patient:general-practitioner&_include:iterate=CareTeam:participant&_include:iterate=Organization:partof&_count=10&_id=smart-MedicationDispense-567

            var refSearchParameter = _searchParameterDefinitionManager.GetSearchParameter("Patient", "general-practitioner");
            var includeIteratePatientGeneralPractitioner = new IncludeExpression("Patient", refSearchParameter, null, false, false, false);

            refSearchParameter = _searchParameterDefinitionManager.GetSearchParameter("MedicationDispense", "prescription");
            var includeMedicationDispensePrescription = new IncludeExpression("MedicationDispense", refSearchParameter, null, false, false, false);

            refSearchParameter = _searchParameterDefinitionManager.GetSearchParameter("CareTeam", "participant");
            var includeCareTeamParticipant = new IncludeExpression("CareTeam", refSearchParameter, null, false, false, true);

            refSearchParameter = _searchParameterDefinitionManager.GetSearchParameter("Organization", "partof");
            var includeOrganizationPartof = new IncludeExpression("Organization", refSearchParameter, null, false, false, true);
            ...
            var sqlExpression = new SqlRootExpression(
                new List<TableExpression>
                {
                    new TableExpression(null, null, denormalizedExpression, TableExpressionKind.All),
                    new TableExpression(IncludeQueryGenerator.Instance,  includeIteratePatientGeneralPractitioner, null, TableExpressionKind.Include),
                    new TableExpression(IncludeQueryGenerator.Instance,  includeCareTeamParticipant, null, TableExpressionKind.Include),
                    new TableExpression(IncludeQueryGenerator.Instance,  includeMedicationDispensePrescription, null, TableExpressionKind.Include),
                    new TableExpression(IncludeQueryGenerator.Instance,  includeOrganizationPartof, null, TableExpressionKind.Include),
                    new TableExpression(null, null, null, TableExpressionKind.Top),
                },
                new List<Expression>());
            ...
            // Assert the number of expressions and their order is correct, including IncludeUnionAll expression, which was added in the IncludeRewriter visit.
            Assert.NotNull(reorderedExpressions);
            Assert.Equal(5, reorderedExpressions.Count);
            ...
            Assert.Equal(TableExpressionKind.IncludeUnionAll, reorderedExpressions[4].Kind);
        }
    }
}