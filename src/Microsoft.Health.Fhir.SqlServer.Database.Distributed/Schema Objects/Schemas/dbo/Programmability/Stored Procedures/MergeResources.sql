...
DECLARE @Mode varchar(100) = 'T='+convert(varchar,@TransactionId)+' RT='+convert(varchar,@ResourceTypeId)+' Rows='+convert(varchar,@InputRows)+' SI='+convert(varchar,@SimpleInsert)+' D='+convert(varchar,@Delete)
...
BEGIN TRY
  ...
  BEGIN
    DELETE FROM A
      ...
      WHERE ResourceTypeId = @ResourceTypeId AND TransactionId = @TransactionId AND Sequence % 10 IN (0,1,2,3,4,5,6,7,8)
      OPTION (MAXDOP 1)
    ...
    DELETE FROM A
      FROM dbo.CompartmentAssignment A WITH (INDEX = 1)
      WHERE ResourceTypeId = @ResourceTypeId AND TransactionId = @TransactionId AND Sequence % 10 IN (0,1,2,3,4,5,6,7,8)
      OPTION (MAXDOP 1)
    SET @AffectedRows = @AffectedRows + @@rowcount
