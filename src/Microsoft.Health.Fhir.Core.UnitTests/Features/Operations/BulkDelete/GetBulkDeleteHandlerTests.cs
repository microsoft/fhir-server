...
namespace Microsoft.Health.Fhir.Core.UnitTests.Features.Operations.BulkDelete
{
    ...
    public class GetBulkDeleteHandlerTests
    {
        ...
        private async Task RunGetBulkDeleteTest(IReadOnlyList<Tuple<JobInfo, int>> jobs, GetBulkDeleteResponse expectedResponse)
        {
            ...
            {
                var definition = JsonConvert.SerializeObject(new BulkDeleteDefinition(JobType.BulkDeleteProcessing, DeleteOperation.HardDelete, null, null, null, "test", "test", "test", job.Item2));
