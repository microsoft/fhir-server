...
@ResourceTypeId SMALLINT, @StartId BIGINT, @EndId BIGINT, @GlobalEndId BIGINT=NULL, @IncludeHistory BIT=0, @IncludeDeleted BIT=0, @Top BIGINT=NULL
AS
SET NOCOUNT ON;
DECLARE @SP AS VARCHAR (100) = 'GetResourcesByTypeAndSurrogateIdRange', @Mode AS VARCHAR (100) = 'RT=' + isnull(CONVERT (VARCHAR, @ResourceTypeId), 'NULL') + ' S=' + isnull(CONVERT (VARCHAR, @StartId), 'NULL') + ' E=' + isnull(CONVERT (VARCHAR, @EndId), 'NULL') + ' GE=' + isnull(CONVERT (VARCHAR, @GlobalEndId), 'NULL') + ' HI=' + isnull(CONVERT (VARCHAR, @IncludeHistory), 'NULL') + ' DE=' + isnull(CONVERT (VARCHAR, @IncludeDeleted), 'NULL') + ' T=' + isnull(CONVERT (VARCHAR, @Top), 'NULL'), @st AS DATETIME = getUTCdate(), @DummyTop AS BIGINT = 9223372036854775807, @TopRows AS BIGINT;
SET @TopRows = CASE WHEN @Top IS NULL
                         OR @Top <= 0 THEN @DummyTop ELSE @Top END;
BEGIN TRY
       ...
       AND @IncludeHistory = 0
        BEGIN
            ...
            IF @@rowcount > 0
                ...
                OPTION (MAXDOP 1, OPTIMIZE FOR (@DummyTop = 1));
        END
    SELECT   TOP (@TopRows) ResourceTypeId,
                            ResourceId,
                            Version,
                            IsDeleted,
                            ResourceSurrogateId,
