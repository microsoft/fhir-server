...
CREATE NONCLUSTERED INDEX IX_ReferenceSearchParam_Compartment
    ON dbo.ReferenceSearchParam(ReferenceResourceId, ReferenceResourceTypeId, SearchParamId)
    INCLUDE(ResourceTypeId, ResourceSurrogateId) WITH (DATA_COMPRESSION = PAGE, ONLINE = ON, RESUMABLE = ON)
    ON PartitionScheme_ResourceTypeId (ResourceTypeId);
