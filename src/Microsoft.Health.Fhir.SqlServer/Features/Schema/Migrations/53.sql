...
SET NOCOUNT ON;
SET XACT_ABORT ON;
DECLARE @SP AS VARCHAR (100) = 'DeleteSearchParameterReferences', @DeletedSearchParams AS INT = 0, @SearchParameterIds AS VARCHAR (MAX), @dt AS DATETIME = getUTCdate();
...
DELETE target
FROM   dbo.DateTimeSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
DELETE target
FROM   dbo.NumberSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
DELETE target
FROM   dbo.ReferenceSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
DELETE target
FROM   dbo.ReferenceTokenCompositeSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
DELETE target
FROM   dbo.StringSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
DELETE target
FROM   dbo.TokenDateTimeCompositeSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
DELETE target
FROM   dbo.TokenQuantityCompositeSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
DELETE target
FROM   dbo.TokenSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
DELETE target
FROM   dbo.TokenStringCompositeSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
DELETE target
FROM   dbo.TokenTokenCompositeSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
DELETE target
FROM   dbo.UriSearchParam AS target
       INNER JOIN
       @SearchParameterTable AS source
       ON target.SearchParamId = source.SearchParamId;
SET @DeletedSearchParams += @@rowcount;
