ALTER PROCEDURE dbo.GetResourcesByTypeAndSurrogateIdRange @ResourceTypeId smallint, @StartId bigint, @EndId bigint, @GlobalEndId bigint = NULL, @IncludeHistory bit = 0, @IncludeDeleted bit = 0, @Top bigint = NULL
AS
set nocount on
DECLARE @SP varchar(100) = 'GetResourcesByTypeAndSurrogateIdRange'
       ,@Mode varchar(100) = 'RT='+isnull(convert(varchar,@ResourceTypeId),'NULL')
                           ...
                           +' T='+isnull(convert(varchar,@Top),'NULL')
       ,@st datetime = getUTCdate()
       ,@DummyTop bigint = 9223372036854775807
       ,@TopRows bigint

SET @TopRows = CASE WHEN @Top IS NULL OR @Top <= 0 THEN @DummyTop ELSE @Top END
...
BEGIN TRY
  ...
  SELECT TOP (@TopRows) ResourceTypeId, ResourceId, Version, IsDeleted, ResourceSurrogateId, RequestMethod, IsMatch, IsPartial, IsRawResourceMetaSet, SearchParamHash, RawResource, IsHistory
    FROM (
      SELECT ResourceTypeId, ResourceId, Version, IsDeleted, ResourceSurrogateId, RequestMethod, IsMatch = convert(bit,1), IsPartial = convert(bit,0), IsRawResourceMetaSet, SearchParamHash, RawResource, IsHistory 
        FROM dbo.Resource
        WHERE ResourceTypeId = @ResourceTypeId 
          AND ResourceSurrogateId BETWEEN @StartId AND @EndId 
    ...
    ORDER BY ResourceSurrogateId, ResourceId, Version
    OPTION (MAXDOP 1)
