        ...
        BEGIN
            ...
            IF EXISTS (SELECT *
                ...
                SELECT ResourceTypeId,
                       ...
                       CodeOverflow
                FROM   (SELECT TOP (@DummyTop) *
                        FROM   @TokenSearchParamHighCards) AS A
                WHERE  EXISTS (SELECT *
                               FROM   @Existing AS B
                               WHERE  B.ResourceTypeId = A.ResourceTypeId
                                      AND B.SurrogateId = A.ResourceSurrogateId)
...
GO
IF object_id('tempdb..#RTs') IS NOT NULL
    DROP TABLE #RTs;
...
AS
SELECT ResourceTypeId,ResourceSurrogateId,SearchParamId,Text,TextOverflow,IsMin,IsMax,IsHistory = convert(bit,0) FROM dbo.StringSearchParam_Partitioned WHERE ResourceTypeId NOT IN (4,14,15,19,28,35,40,44,53,61,62,76,79,96,100,103,108,110,138)', @InsertTrigger AS VARCHAR (MAX) = '
...
WHILE EXISTS (SELECT *
    ...
    BEGIN
        ...
        IF NOT EXISTS (SELECT *
                       ...
                       WHERE  name = 'DF_StringSearchParam_' + @RT + '_IsHistory')
...
AS
SELECT ResourceTypeId,ResourceSurrogateId,SearchParamId,SystemId,Code,CodeOverflow, IsHistory = convert(bit,0) FROM dbo.TokenSearchParam_Partitioned WHERE ResourceTypeId NOT IN (4,14,15,19,28,35,40,44,53,61,62,76,79,96,100,103,108,110,138)', @InsertTrigger AS VARCHAR (MAX) = '
...
WHILE EXISTS (SELECT *
    ...
    BEGIN
        ...
        IF NOT EXISTS (SELECT *
                       FROM   sys.default_constraints
                       WHERE  name = 'DF_TokenSearchParam_' + @RT + '_IsHistory')
