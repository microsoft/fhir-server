...
CREATE CLUSTERED INDEX IXC_ResourceSurrogateId_SearchParamId_ResourceTypeId
    ON dbo.ReferenceSearchParam(ResourceSurrogateId, SearchParamId, ResourceTypeId) WITH (DATA_COMPRESSION = PAGE)
    ON PartitionScheme_ResourceTypeId (ResourceTypeId);

CREATE UNIQUE INDEX IXU_ReferenceResourceIdInt_ReferenceResourceId_ReferenceResourceTypeId_SearchParamId_BaseUri_ResourceSurrogateId_ResourceTypeId
    ON dbo.ReferenceSearchParam(ReferenceResourceIdInt, ReferenceResourceId, ReferenceResourceTypeId, SearchParamId, BaseUri, ResourceSurrogateId, ResourceTypeId) WITH (DATA_COMPRESSION = PAGE)
    ON PartitionScheme_ResourceTypeId (ResourceTypeId);
...
CREATE TABLE dbo.Resource (
    ResourceTypeId       SMALLINT        NOT NULL,
    ResourceId           VARCHAR (64)    COLLATE Latin1_General_100_CS_AS CONSTRAINT DF_Resource_ResourceId DEFAULT '' NOT NULL,
    ResourceIdInt        BIGINT          CONSTRAINT DF_Resource_ResourceIdInt DEFAULT 0 NOT NULL,
    Version              INT             NOT NULL,
    IsHistory            BIT             NOT NULL,
    ResourceSurrogateId  BIGINT          NOT NULL,
    IsDeleted            BIT             NOT NULL,
    ...
    OffsetInFile         INT             NULL CONSTRAINT PKC_Resource_ResourceSurrogateId_ResourceTypeId PRIMARY KEY CLUSTERED (ResourceSurrogateId, ResourceTypeId) WITH (DATA_COMPRESSION = PAGE) ON PartitionScheme_ResourceTypeId (ResourceTypeId),
    CONSTRAINT CH_Resource_RawResource_OffsetInFile CHECK (RawResource IS NOT NULL
                                                           OR OffsetInFile IS NOT NULL),
    CONSTRAINT CH_Resource_ResourceIdInt_ResourceId CHECK (ResourceIdInt = 0
                                                           AND ResourceId <> ''
                                                           OR ResourceIdInt <> 0
                                                              AND ResourceId = '')
);
...
CREATE INDEX IX_TransactionId_ResourceTypeId_WHERE_TransactionId_NOT_NULL
    ON dbo.Resource(TransactionId, ResourceTypeId) WHERE TransactionId IS NOT NULL WITH (DATA_COMPRESSION = PAGE)
    ON PartitionScheme_ResourceTypeId (ResourceTypeId);

CREATE INDEX IX_HistoryTransactionId_ResourceTypeId_WHERE_HistoryTransactionId_NOT_NULL
    ON dbo.Resource(HistoryTransactionId, ResourceTypeId) WHERE HistoryTransactionId IS NOT NULL WITH (DATA_COMPRESSION = PAGE)
    ON PartitionScheme_ResourceTypeId (ResourceTypeId);

CREATE UNIQUE INDEX IXU_ResourceIdInt_ResourceId_Version_ResourceTypeId
    ON dbo.Resource(ResourceIdInt, ResourceId, Version, ResourceTypeId) WITH (DATA_COMPRESSION = PAGE)
    ON PartitionScheme_ResourceTypeId (ResourceTypeId);

CREATE UNIQUE INDEX IXU_ResourceIdInt_ResourceId_ResourceTypeId_INCLUDE_Version_IsDeleted_WHERE_IsHistory_0
...
CREATE UNIQUE INDEX IX_Resource_ResourceTypeId_ResourceSurrgateId
    ...
    IF @NotNullVersionExists = 1
        IF @NullVersionExists = 0
            ...
            FROM   (SELECT TOP (@DummyTop) *
                   ...
                   INNER JOIN
                   dbo.ResourceTbl AS C WITH (INDEX (IXU_ResourceIdInt_ResourceId_Version_ResourceTypeId))
            ...
            FROM   (SELECT B.ResourceTypeId,
                    ...
                    FROM   (SELECT TOP (@DummyTop) *
                           ...
                           ON B.ResourceTypeId = A.ResourceTypeId
                              AND B.ResourceId = A.ResourceId
                           INNER JOIN
                           dbo.ResourceTbl AS C WITH (INDEX (IXU_ResourceIdInt_ResourceId_ResourceTypeId_INCLUDE_Version_IsDeleted_WHERE_IsHistory_0))
                           ON C.ResourceTypeId = A.ResourceTypeId
                              AND C.ResourceIdInt = B.ResourceIdInt
                              AND C.ResourceId = ''
...
SET @AffectedRows = 0;
BEGIN TRY
    DECLARE @RTs AS TABLE (
        ResourceTypeId SMALLINT NOT NULL PRIMARY KEY);
    ...
    WHILE EXISTS (SELECT *
                  FROM   @RTs)
        BEGIN
            SET @RT = (SELECT TOP 1 ResourceTypeId
                       FROM   @RTs);
            INSERT INTO @ExistingIds (ResourceTypeId, ResourceIdInt, ResourceId)
            SELECT @RT,
                   ResourceIdInt,
                   A.ResourceId
            FROM   (SELECT *
                    FROM   @InputIds
                    WHERE  ResourceTypeId = @RT) AS A
                   INNER JOIN
                   dbo.ResourceIdIntMap AS B
                   ON B.ResourceTypeId = @RT
                      AND B.ResourceId = A.ResourceId;
            DELETE @InsertIds;
            INSERT INTO @InsertIds (ResourceIndex, ResourceId)
            SELECT row_number() OVER (ORDER BY ResourceId) - 1 AS RowId,
                   ResourceId
            FROM   (SELECT ResourceId
                    FROM   @InputIds
                    WHERE  ResourceTypeId = @RT) AS A
            WHERE  NOT EXISTS (SELECT *
                               FROM   @ExistingIds AS B
                               WHERE  B.ResourceTypeId = @RT
                                      AND B.ResourceId = A.ResourceId);
    ...
    WHILE EXISTS (SELECT *
        ...
        BEGIN
            ...
            WHERE  ResourceTypeId = @RT;
        END
    INSERT INTO @ResourcesWithIds (ResourceTypeId, ResourceIdInt, Version, IsHistory, ResourceSurrogateId, IsDeleted, RequestMethod, RawResource, IsRawResourceMetaSet, SearchParamHash, OffsetInFile)
    SELECT A.ResourceTypeId,
           isnull(C.ResourceIdInt, B.ResourceIdInt),
           Version,
           IsHistory,
        ...
        BEGIN
            ...
            IF @@rowcount > 0
                BEGIN
                    ...
                    SET @AffectedRows += @@rowcount;
                END
            INSERT INTO dbo.ResourceTbl (ResourceTypeId, ResourceIdInt, Version, IsHistory, ResourceSurrogateId, IsDeleted, RequestMethod, RawResource, IsRawResourceMetaSet, SearchParamHash, TransactionId, OffsetInFile)
            SELECT ResourceTypeId,
                   ResourceIdInt,
                   Version,
                   IsHistory,
    ...
    ELSE
        BEGIN
            ...
            WHERE  EXISTS (SELECT *
                   ...
                   AND NOT EXISTS (SELECT *
                                   FROM   dbo.ReferenceSearchParamTbl AS C
...
BEGIN TRY
    ...
    FROM   @Ids AS A
           ...
           ON B.ResourceSurrogateId = A.ResourceSurrogateId;
    DELETE B
    FROM   dbo.ReferenceSearchParam AS B
    WHERE  EXISTS (SELECT *
                   FROM   @Ids AS A
                   WHERE  A.ResourceTypeId = B.ResourceTypeId
                          AND A.ResourceSurrogateId = B.ResourceSurrogateId);
...
SELECT A.ResourceTypeId,
       ...
       BaseUri,
       ReferenceResourceTypeId,
       CASE WHEN A.ReferenceResourceId = '' THEN B.ResourceId ELSE A.ReferenceResourceId END AS ReferenceResourceId,
       ReferenceResourceVersion
...
CREATE OR ALTER TRIGGER dbo.ReferenceSearchParamIns
    ...
    INSTEAD OF INSERT
    AS DECLARE @DummyTop AS BIGINT = 9223372036854775807;
       BEGIN
           INSERT INTO dbo.ReferenceSearchParamTbl (ResourceTypeId, ResourceSurrogateId, SearchParamId, BaseUri, ReferenceResourceTypeId, ReferenceResourceIdInt, ReferenceResourceVersion)
           SELECT A.ResourceTypeId,
                  ResourceSurrogateId,
...
SELECT A.ResourceTypeId,
       ResourceSurrogateId,
       CASE WHEN A.ResourceId = '' THEN B.ResourceId ELSE A.ResourceId END AS ResourceId,
       Version,
       IsHistory,
       IsDeleted,
       RequestMethod,
