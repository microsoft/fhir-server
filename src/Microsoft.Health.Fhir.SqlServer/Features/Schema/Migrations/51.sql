...
BEGIN TRY
    ...
    SET @AffectedRows += @@rowcount;
    INSERT INTO dbo.CompartmentAssignment (ResourceTypeId, ResourceSurrogateId, CompartmentTypeId, ReferenceResourceId)
    SELECT ResourceTypeId,
           ResourceSurrogateId,
           CompartmentTypeId,
           ReferenceResourceId
...
GO
CREATE PROCEDURE dbo.QueryIdentifierWithIncludeAndOtherParam
@p0 SMALLINT, @p1 SMALLINT, @p2 SMALLINT, @p3 SMALLINT, @p4 INT, @p5 VARCHAR (256), @p6 SMALLINT, @p7 VARCHAR (256), @p8 INT, @p9 SMALLINT, @p10 INT
AS
WITH     cte0
AS       (SELECT refSource.ResourceTypeId AS T1,
                 refSource.ResourceSurrogateId AS Sid1,
                 refTarget.ResourceTypeId AS T2,
                 refTarget.ResourceSurrogateId AS Sid2
          FROM   dbo.ReferenceSearchParam AS refSource
                 INNER JOIN
                 dbo.Resource AS refTarget
...
AS       (SELECT refSource.ResourceTypeId AS T1,
          ...
          WHERE  refSource.SearchParamId = @p0
                 ...
                 AND EXISTS (SELECT *
                             FROM   cte1
                             WHERE  refSource.ResourceTypeId = T1
                                    AND refSource.ResourceSurrogateId = Sid1)
                 AND refSource.ResourceTypeId = @p1),
         cte3
AS       (SELECT T1,
                 Sid1,
          ...
          WHERE  IsHistory = 0
                 ...
                 AND Code = @p7),
         cte4
AS       (SELECT ROW_NUMBER() OVER (ORDER BY T1 ASC, Sid1 ASC) AS Row,
                 *
          FROM   (SELECT   DISTINCT TOP (@p8) T1,
                                              Sid1,
                                              1 AS IsMatch,
                                              0 AS IsPartial
                  FROM     cte3
                  ORDER BY T1 ASC, Sid1 ASC) AS t),
         cte5
AS       (SELECT DISTINCT refTarget.ResourceTypeId AS T1,
...
SELECT   DISTINCT r.ResourceTypeId,
                  ...
                  r.RequestMethod,
                  CAST (IsMatch AS BIT) AS IsMatch,
                  CAST (IsPartial AS BIT) AS IsPartial,
...
FROM     dbo.Resource AS r
         INNER JOIN
         cte7
         ON r.ResourceTypeId = cte7.T1
            AND r.ResourceSurrogateId = cte7.Sid1
