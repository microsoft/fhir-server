...
CREATE TABLE dbo.Resource (
    ...
    TransactionId        BIGINT           NULL,
    HistoryTransactionId BIGINT           NULL,
    DecompressedLength   DECIMAL (36, 18) NULL CONSTRAINT PKC_Resource PRIMARY KEY CLUSTERED (ResourceTypeId, ResourceSurrogateId) WITH (DATA_COMPRESSION = PAGE) ON PartitionScheme_ResourceTypeId (ResourceTypeId),
...
BEGIN TRY
    ...
    IF @DecompressedOverridesJson IS NOT NULL
        BEGIN
            ...
            SELECT ResourceSurrogateId,
                   DecompressedLength
            FROM   OPENJSON (@DecompressedOverridesJson) WITH (ResourceSurrogateId BIGINT '$.ResourceSurrogateId', DecompressedLength DECIMAL (36, 18) '$.DecompressedLength');
        END
    ...
    IF @IsRetry = 0
        BEGIN
            ...
            SELECT ResourceTypeId,
                   ...
                   SearchParamHash,
                   @TransactionId,
                   OV.DecompressedLength
            FROM   @Resources AS R
                   LEFT OUTER JOIN
                   @DecompressedOverrides AS OV
