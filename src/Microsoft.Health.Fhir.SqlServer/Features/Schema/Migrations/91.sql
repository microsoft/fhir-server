...
AS
SET NOCOUNT ON;
DECLARE @SP AS VARCHAR (100) = 'UpdateSqlStatistics', @Mode AS VARCHAR (100) = '', @st AS DATETIME = getUTCdate(), @TableName AS SYSNAME, @SQL AS NVARCHAR (500), @TotalTables AS INT = 0, @UpdatedTables AS INT = 0;
BEGIN TRY
    EXECUTE dbo.LogEvent @Process = @SP, @Status = 'Start', @Mode = @Mode;
