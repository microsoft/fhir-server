...
{
    internal class SqlResourceImporter : IResourceImporter
    {
        ...
        private ILogger<SqlResourceImporter> _logger;

        public SqlResourceImporter(
            ISqlImportOperation sqlImportOperation,
            SqlServerFhirModel sqlServerFhirModel,
            IImportErrorSerializer importErrorSerializer,
            IOptions<OperationsConfiguration> operationsConfig,
            ILogger<SqlResourceImporter> logger)
        {
        ...
        private async Task ImportInternalAsync(Channel<ImportResource> inputChannel, Channel<ImportProcessingProgress> outputChannel, IImportErrorStore importErrorStore, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var progress = await UploadImportErrorsAsync(importErrorStore, succeedCount, failedCount, importErrorBuffer.ToArray(), currentIndex, cancellationToken);
