...
namespace Microsoft.Health.Fhir.SqlServer.Features.Search.Expressions.Visitors.QueryGenerators
{
    internal class SqlQueryGenerator : DefaultSqlExpressionVisitor<SearchOptions, object>
    {
        ...
        // Tracks, in generation (CTE-emission) order, the kind and output CTE index of each top-level
        // logical SearchParamTableExpression as it is generated by the main CTE loop. This is the
        // authoritative source for resolving the restricting predecessor CTE of a JOIN because:
        //   1. A UnionExpression expands a single logical expression into multiple CTEs, so the raw
        //      _tableExpressionCounter runs ahead of the index into _rootExpression.SearchParamTableExpressions.
        //   2. SortExpressionsByQueryLogic reorders expressions (unions first), so generation order
        //      differs from the original _rootExpression.SearchParamTableExpressions order.
        private readonly List<(SearchParamTableExpressionKind Kind, int OutputCteIndex)> _generatedTableExpressions = new();
            ...
            if (expression.SearchParamTableExpressions.Count > 0)
            {
                ...
                StringBuilder.AppendDelimited($"{Environment.NewLine},", expression.SearchParamTableExpressions.SortExpressionsByQueryLogic(), (sb, tableExpression) =>
                {
                    if (tableExpression.SplitExpressions(out UnionExpression unionExpression, out SearchParamTableExpression allOtherRemainingExpressions))
                    {
                        ...
                        // Record the union block's output (aggregate) CTE in generation order. This must be
                        // recorded BEFORE allOtherRemainingExpressions below so that the trailing expression
                        // resolves its restricting predecessor to this union aggregate rather than to the
                        // previous top-level expression. FindRestrictingPredecessorTableExpressionIndex walks
                        // this list, which is what keeps predecessor JOINs correct once a union inflates the
                        // CTE counter (and keeps both branches of a ConcatenationRewriter pair joined to their
                        // shared predecessor).
                        _generatedTableExpressions.Add((tableExpression.Kind, _tableExpressionCounter));
                        ...
                        if ((!hasSmartV2UnionExpressionInTheSet && allOtherRemainingExpressions != null) || (hasSmartV2UnionExpressionInTheSet && allOtherRemainingExpressions != null && numberOfUnionExpressions == 0))
                        {
                            ...
                            // Record the trailing non-union expression's output CTE in generation order.
                            _generatedTableExpressions.Add((allOtherRemainingExpressions.Kind, _tableExpressionCounter));
                        }
                    }
                    else
                    {
                        ...
                        // Record this expression's output CTE in generation order.
                        _generatedTableExpressions.Add((tableExpression.Kind, _tableExpressionCounter));
                    }
                });
        ...
        private int FindRestrictingPredecessorTableExpressionIndex(SearchParamTableExpressionKind currentKind)
        {
            ...
            // being generated, walking _generatedTableExpressions in generation (CTE-emission) order.
            //
            // The current expression has not yet been recorded into _generatedTableExpressions, so the
            // last recorded entry is its immediate predecessor.
            //
            // A Concatenation branch is the second half of a UNION ALL pair produced by the
            // ConcatenationRewriter. It must restrict against the SAME predecessor as its immediately
            // preceding Normal sibling, so we resolve the sibling's predecessor instead of joining the
            // sibling itself. This mirrors the original recursive behavior, but operates on generation
            // order rather than on indices into the unsorted _rootExpression.SearchParamTableExpressions,
            // which is what corrects predecessor selection once a union inflates the CTE counter.
            int ResolvePredecessor(int position, SearchParamTableExpressionKind kind)
            {
                if (kind == SearchParamTableExpressionKind.Concatenation)
                {
                    ...
                    return ResolvePredecessor(siblingPosition, _generatedTableExpressions[siblingPosition].Kind);
                }

                int predecessorPosition = position - 1;
                return predecessorPosition >= 0 ? _generatedTableExpressions[predecessorPosition].OutputCteIndex : -1;
            }

            return ResolvePredecessor(_generatedTableExpressions.Count, currentKind);
        }
