...
namespace Microsoft.Health.Fhir.SqlServer.Features.Watchdogs
{
    internal class InvisibleHistoryCleanupWatchdog : Watchdog<InvisibleHistoryCleanupWatchdog>
    {
        ...
        protected override async Task ExecuteAsync()
        {
            ...
            var visibility = await _store.SqlStoreClient.MergeResourcesGetTransactionVisibilityAsync(_cancellationToken);
            _logger.LogInformation($"{Name}: last cleaned up transaction={lastTranId} visibility={visibility}.");

            var transToClean = await _store.SqlStoreClient.GetTransactionsAsync(lastTranId, visibility, _cancellationToken, DateTime.UtcNow.AddDays((-1) * _retentionPeriodDays));
            ...
            {
                var rows = await _store.SqlStoreClient.MergeResourcesDeleteInvisibleHistory(tran.TransactionId, _cancellationToken);
                _logger.LogInformation($"{Name}: transaction={tran.TransactionId} removed rows={rows}.");
                totalRows += rows;

                await _store.SqlStoreClient.MergeResourcesPutTransactionInvisibleHistoryAsync(tran.TransactionId, _cancellationToken);
            }
