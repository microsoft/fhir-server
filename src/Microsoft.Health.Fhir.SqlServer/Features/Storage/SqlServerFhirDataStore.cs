            ...
            bool requireETagOnUpdate = false)
        {
            ...
            using (var stream = new RecyclableMemoryStream(_memoryStreamManager))
            {
                ...
                stream.Seek(0, 0);
                    ...
                    if (stream.Length < 50) // rather than error on a length of 0, a stream with a small number of bytes should still throw an error. RawResource.Data = null, still results in a stream of 29 bytes
                    {
