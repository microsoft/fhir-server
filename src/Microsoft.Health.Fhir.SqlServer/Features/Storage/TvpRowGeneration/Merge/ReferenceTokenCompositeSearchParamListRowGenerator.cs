...
namespace Microsoft.Health.Fhir.SqlServer.Features.Storage.TvpRowGeneration
{
    internal class ReferenceTokenCompositeSearchParamListRowGenerator : CompositeSearchParamRowGenerator<(ReferenceSearchValue component1, TokenSearchValue component2), ReferenceTokenCompositeSearchParamListRow>
    {
        ...
        internal IEnumerable<string> GenerateCSVs(IReadOnlyList<MergeResourceWrapper> resources)
        {
            ...
            {
                yield return $"{row.ResourceTypeId},{row.ResourceSurrogateId},{row.SearchParamId},{row.BaseUri1},{row.ReferenceResourceTypeId1},{row.ReferenceResourceId1},{row.ReferenceResourceVersion1},{row.SystemId2},{row.Code2}";
            }
        }
    }
}
