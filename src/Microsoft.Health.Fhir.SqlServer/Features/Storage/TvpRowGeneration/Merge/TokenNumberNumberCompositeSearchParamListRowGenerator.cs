...
namespace Microsoft.Health.Fhir.SqlServer.Features.Storage.TvpRowGeneration
{
    internal class TokenNumberNumberCompositeSearchParamListRowGenerator : CompositeSearchParamRowGenerator<(TokenSearchValue component1, NumberSearchValue component2, NumberSearchValue component3), TokenNumberNumberCompositeSearchParamListRow>
    {
        ...
        internal IEnumerable<string> GenerateCSVs(IReadOnlyList<MergeResourceWrapper> resources)
        {
            ...
            {
                yield return $"{row.ResourceTypeId},{row.ResourceSurrogateId},{row.SearchParamId},{row.SystemId1},{row.Code1},{row.SingleValue2},{row.LowValue2},{row.HighValue2},{row.LowValue3},{row.HighValue3},{row.HasRange}";
            }
        }
    }
}
