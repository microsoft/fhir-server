...
{
    internal abstract class BulkSearchParameterRowGenerator<TSearchValue, TRow> : ITableValuedParameterRowGenerator<IReadOnlyList<ResourceMetadata>, TRow>
        where TRow : struct
    {
        private readonly bool _isConvertSearchValueOverridden;
        private bool _isInitialized;

        protected BulkSearchParameterRowGenerator(SqlServerFhirModel model)
        {
        ...
        public virtual IEnumerable<TRow> GenerateRows(IReadOnlyList<ResourceMetadata> input)
        {
            EnsureInitialized();
        ...
        internal abstract bool TryGenerateRow(int resourceId, short searchParamId, TSearchValue searchValue, out TRow row);
    }
}
