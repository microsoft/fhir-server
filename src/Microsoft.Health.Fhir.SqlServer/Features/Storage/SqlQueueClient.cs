...
namespace Microsoft.Health.Fhir.SqlServer.Features.Storage
{
    public class SqlQueueClient : IQueueClient
    {
        ...
        {
            await SqlServerFhirDataStore.ExecuteWithRetries(
                "CompleteJobAsync",
                async () =>
                {
                    using SqlConnectionWrapper sqlConnectionWrapper = await _sqlConnectionWrapperFactory.ObtainSqlConnectionWrapperAsync(cancellationToken, true);
        ...
        public async Task<JobInfo> DequeueAsync(byte queueType, string worker, int heartbeatTimeoutSec, CancellationToken cancellationToken, long? jobId = null)
        {
            ...
            await SqlServerFhirDataStore.ExecuteWithRetries(
                ...
                async () =>
                {
                    ...
                    jobInfo = await sqlDataReader.ReadJobInfoAsync(cancellationToken);
                    if (jobInfo != null)
                    {
        ...
        {
            IReadOnlyList<JobInfo> results = null;
            await SqlServerFhirDataStore.ExecuteWithRetries(
                "EnqueueAsync",
                async () =>
                {
                    ...
                    try
                    {
                        ...
                        results = await sqlDataReader.ReadJobInfosAsync(cancellationToken);
                    }
                    catch (SqlException sqlEx)
                    {
                        if (sqlEx.State == 127)
        ...
        public async Task<JobInfo> GetJobByIdAsync(byte queueType, long jobId, bool returnDefinition, CancellationToken cancellationToken)
        {
            ...
            await SqlServerFhirDataStore.ExecuteWithRetries(
                ...
                async () =>
                {
                    ...
                    result = await sqlDataReader.ReadJobInfoAsync(cancellationToken);
                });
            return result;
        }
