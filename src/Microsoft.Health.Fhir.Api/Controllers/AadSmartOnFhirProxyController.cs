    ...
    public class AadSmartOnFhirProxyController : Controller
    {
        ...
        public AadSmartOnFhirProxyController(IOptions<SecurityConfiguration> securityConfigurationOptions, IHttpClientFactory httpClientFactory, IUrlResolver urlResolver, ILogger<SecurityConfiguration> logger)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    logger.LogWarning(ex, "There was an exception while attempting to read the OpenId Configuration from \"{0}\".", openIdConfigurationUrl);
            ...
            {
                logger.LogError("There was an error attempting to read the endpoints from \"{0}\".", openIdConfigurationUrl);
                throw new OpenIdConfigurationException();
            }
        }
            ...
            [FromQuery(Name = "error_description")] string errorDescription)
        {
            ...
            {
                _logger.LogError("Error parsing launch parameters: {0}", ex.Message);
            ...
            [FromForm(Name = "client_secret")] string clientSecret)
        {
            ...
            {
                _logger.LogError("Error decoding compound code: {0}", ex.Message);
