...
namespace Microsoft.Health.Fhir.Core.Features.Operations.Reindex
{
    public class ReindexUtilities
    {
        ...
        {
            var updatedResources = new List<SearchResultEntry>();
            ...
            using (IScoped<IFhirDataStore> store = _fhirDataStoreFactory())
            {
                foreach (var entry in results.Results)
                {
                    ...
                    var previousIndicesHash = new HashSet<SearchIndexEntry>(entry.Resource.SearchIndices);
                    if (!newIndicesHash.SetEquals(previousIndicesHash))
                    {
                        entry.Resource.SearchIndices = newIndices;
                    }
