...
namespace Microsoft.Health.Fhir.Core.UnitTests.Features.Search
{
    public class SearchParameterDefinitionManagerTests : IAsyncLifetime
    {
        ...
        public SearchParameterDefinitionManagerTests()
        {
            ...
            _searchParameterDefinitionManager = new SearchParameterDefinitionManager(ModelInfoProvider.Instance, () => _searchService.CreateMockScope());
        ...
        public async Task GivenExistingSearchParameters_WhenStartingSearchParameterDefinitionManager_ThenExistingParametersAdded()
        {
            ...
            var searchParameterDefinitionManagerExistingParams = new SearchParameterDefinitionManager(ModelInfoProvider.Instance, () => searchService.CreateMockScope());
            await searchParameterDefinitionManagerExistingParams.StartAsync(CancellationToken.None);

            var patientParams = searchParameterDefinitionManagerExistingParams.GetSearchParameters("Patient");
            Assert.True(patientParams.Where(p => p.Name == "preexisting").Any());
            Assert.True(patientParams.Where(p => p.Name == "preexisting2").Any());
        }
