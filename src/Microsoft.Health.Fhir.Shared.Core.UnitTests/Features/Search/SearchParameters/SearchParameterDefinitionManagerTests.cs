...
namespace Microsoft.Health.Fhir.Core.UnitTests.Features.Search
{
    ...
    public class SearchParameterDefinitionManagerTests : IAsyncLifetime
    {
        ...
        [Fact]
        public async Task GivenUnresolvedUrlsAcrossMultipleChunks_WhenApplyingSearchParameterUpdates_ThenFallbackScanRunsOnce()
        {
            var urls = Enumerable.Range(0, 250)
                .Select(i => $"http://test/SearchParameter/fallback-{i}")
                .ToList();

            var statuses = urls
                .Select(url => new ResourceSearchParameterStatus
                {
                    Status = SearchParameterStatus.Enabled,
                    Uri = new Uri(url),
            ...
            _searchParameterStatusDataStore.GetSearchParameterStatuses(Arg.Any<CancellationToken>(), Arg.Any<DateTimeOffset?>())
                .Returns(statuses);

            var searchParamsByUrl = urls
                .Select((url, i) => CreateSearchParameter(url, i))
            ...
            searchService.SearchAsync(
                    KnownResourceTypes.SearchParameter,
                    Arg.Is<IReadOnlyList<Tuple<string, string>>>(q => q.All(p => !string.Equals(p.Item1, "url", StringComparison.Ordinal))),
                    Arg.Any<CancellationToken>(),
                    Arg.Any<bool>(),
                    Arg.Any<ResourceVersionType>(),
                    Arg.Any<bool>(),
                    Arg.Any<bool>())
                .Returns(GetSearchResultFromSearchParams(searchParamsByUrl.Values, continuationToken: null));

            var searchParameterDataStoreValidator = Substitute.For<IDataStoreSearchParameterValidator>();
            searchParameterDataStoreValidator.ValidateSearchParameter(Arg.Any<SearchParameterInfo>(), out Arg.Any<string>()).Returns(true, null);

            var operations = new SearchParameterOperations(
                _manager,
                _searchParameterDefinitionManager,
                ModelInfoProvider.Instance,
                _searchParameterSupportResolver,
            ...
            await operations.GetAndApplySearchParameterUpdates(CancellationToken.None, forceFullRefresh: true);

            await searchService.Received(3).SearchAsync(
                KnownResourceTypes.SearchParameter,
                Arg.Is<IReadOnlyList<Tuple<string, string>>>(q => q.Any(p => string.Equals(p.Item1, "url", StringComparison.Ordinal))),
                Arg.Any<CancellationToken>(),
                Arg.Any<bool>(),
        ...
        [Fact]
        public async Task GivenDirectUrlLookupResolvesAll_WhenApplyingSearchParameterUpdates_ThenFallbackScanDoesNotRun()
        {
            var urls = Enumerable.Range(0, 150)
                .Select(i => $"http://test/SearchParameter/direct-{i}")
                .ToList();
            ...
            searchService.SearchAsync(
                ...
                .Returns(callInfo =>
                {
                    var query = callInfo.ArgAt<IReadOnlyList<Tuple<string, string>>>(1);
                    var urlValue = query.Single(p => string.Equals(p.Item1, "url", StringComparison.Ordinal)).Item2;
                    var chunkUrls = urlValue.Split(',', StringSplitOptions.RemoveEmptyEntries);
                    var chunkParams = chunkUrls.Select(url => searchParamsByUrl[url]);
                    return GetSearchResultFromSearchParams(chunkParams, continuationToken: null);
                });
        ...
        private static SearchResult GetSearchResultFromSearchParams(IEnumerable<SearchParameter> searchParams, string continuationToken)
        {
            var serializer = new FhirJsonSerializer();
            var entries = searchParams.Select(searchParam =>
            {
                var rawResource = new RawResource(
                    serializer.SerializeToString(searchParam),
                    FhirResourceFormat.Json,
                ...
                var wrapper = new ResourceWrapper(
                    new ResourceElement(rawResource.ToITypedElement(ModelInfoProvider.Instance)),
                    rawResource,
                    new ResourceRequest("POST"),
                    false,
                    null,
            ...
            return new SearchResult(entries, continuationToken, null, new List<Tuple<string, string>>());
        }

        private static SearchParameter CreateSearchParameter(string url, int index)
        {
            return new SearchParameter
            {
                Id = $"search-parameter-{index}",
                Url = url,
                Type = Hl7.Fhir.Model.SearchParamType.String,
#if Stu3 || R4 || R4B
                Base = new List<ResourceType?> { ResourceType.Patient },
