...
namespace Microsoft.Health.Fhir.Core.UnitTests.Features.Resources
{
    ...
    public partial class ResourceHandlerTests
    {
        ...
        public ResourceHandlerTests()
        {
            ...
            collection.Add(x => new ConditionalCreateResourceHandler(_fhirDataStore, lazyConformanceProvider, _resourceWrapperFactory, _searchService, x.GetService<IMediator>(), () => true, ModelInfoProvider.Instance)).Singleton().AsSelf().AsImplementedInterfaces();
            collection.Add(x => new ConditionalUpsertResourceHandler(_fhirDataStore, lazyConformanceProvider, _resourceWrapperFactory, _searchService, x.GetService<IMediator>(), () => true, ModelInfoProvider.Instance)).Singleton().AsSelf().AsImplementedInterfaces();
