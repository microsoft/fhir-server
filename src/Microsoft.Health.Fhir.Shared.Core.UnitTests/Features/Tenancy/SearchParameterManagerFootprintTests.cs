...
namespace Microsoft.Health.Fhir.Core.UnitTests.Features.Tenancy
{
    ...
    public class SearchParameterManagerFootprintTests
    {
        ...
        // Kept modest so the suite stays fast in CI while still giving a defensible, outlier-resistant
        // median across independent trials rather than a single noisy sample.
        ...
        // One shared, inert set of NSubstitute dependencies reused by every manager constructed in this
        // fixture. None of these substitutes hold or mutate manager-specific lookup state, so constructing
        // 100+ managers from them never inflates the substitute graph itself - only UrlLookup/TypeLookup do.
        ...
        private static readonly ISearchParameterComparer<SearchParameterInfo> SharedComparer =
        ...
        public void GivenRepeatedTrials_WhenManyManagersAreConstructedOverSharedDependencies_ThenMedianManagedMemoryPerManagerIsReportedHonestly()
        {
            ...
            for (int trial = 0; trial < TrialCount; trial++)
            {
                ...
                double perManagerBytes = (after - before) / (double)ManagersPerTrial;
                perManagerBytesByTrial.Add(perManagerBytes);
            ...
            double median = Median(perManagerBytesByTrial);
            ...
            _output.WriteLine($"Per-trial managed-memory delta per manager (bytes): [{string.Join(", ", perManagerBytesByTrial.Select(b => b.ToString("F0")))}]");
            _output.WriteLine($"Median managed-memory projection per manager (bytes): {median:F0}");
        ...
        // single shared, inert dependency graph declared above so that graph's allocation is never counted
        // as per-manager growth.
