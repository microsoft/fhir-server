...
{
    /// <summary>
    /// Handler for conditional update request with "X-Provenance header".
    /// Conditionally updates resource and Provenance resource provided in "X-Provenance" header with it's target as resource.
    /// </summary>
    public sealed class ProvenanceHeaderConditionalUpsertHandler : IRequestHandler<ProvenanceHeaderConditionalUpsertRequest, UpsertResourceResponse>
    {
        private readonly FhirJsonParser _fhirJsonParser;
        private readonly IMediator _mediator;

        public ProvenanceHeaderConditionalUpsertHandler(FhirJsonParser fhirJsonParser, IMediator mediator)
        {
        ...
        public async Task<UpsertResourceResponse> Handle(ProvenanceHeaderConditionalUpsertRequest request, CancellationToken cancellationToken)
        {
            ...
            UpsertResourceResponse targetResource = await _mediator.Send<UpsertResourceResponse>(new ConditionalUpsertResourceRequest(request.Target, request.ConditionalParameters), cancellationToken);
