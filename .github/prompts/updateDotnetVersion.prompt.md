...
Update all three locations with the appropriate versions:
...
4. **Runtime Independence**: The runtime version doesn't need to exactly match SDK version, but should be the latest compatible version
...
2. Runtime deployments should use the updated ASP.NET runtime image

## Summary

To update .NET to the latest patch version:
1. Find the latest SDK version for .NET 9
2. Update `global.json` (main SDK)
3. Update `build/docker/Dockerfile` (SDK and runtime images)
4. Verify all versions match and images exist in MCR
