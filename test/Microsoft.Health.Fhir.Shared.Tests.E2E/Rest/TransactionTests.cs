...
namespace Microsoft.Health.Fhir.Tests.E2E.Rest
{
    ...
    public class TransactionTests : IClassFixture<HttpIntegrationTestFixture>
    {
        ...
        public async Task GivenABundleWithVersionedReference_WhenSubmittingATransaction_ThenResolvedReferenceIsVersionSpecific()
        {
            ...
            // Reference should exactly match to the full url of a resource.
            Assert.Equal(fullUrlBeforeTransaction, referenceBeforeTransaction);
        ...
        [Fact]
        [Trait(Traits.Priority, Priority.One)]
        public async Task GivenABundleWithVersionedReference_WhenSubmittingATransaction_ThenUpdatedReferenceIsVersionSpecific()
        {
            // Insert resources first inorder to test a delete.
            var resource = Samples.GetJsonSample("PatientWithMinimalData");
            FhirResponse<Patient> response = await Client.CreateAsync(resource.ToPoco<Patient>());
            ...
            requestBundle.Entry[0].FullUrl = $"Patient/{insertedId}/_history/5";
            requestBundle.Entry[0].Resource.Id = insertedId;
            requestBundle.Entry[0].Request.Method = HTTPVerb.PUT;
            requestBundle.Entry[0].Request.Url = $"Patient/{insertedId}";

            requestBundle.Entry[1].Resource.GetAllChildren<ResourceReference>().ToList()[0].Reference = $"Patient/{insertedId}/_history/5";
            ...
            var idAfterTransaction = fhirResponse.Resource.Entry[0].Resource.Id;
            var versionIdAfterTransaction = fhirResponse.Resource.Entry[0].Resource.Meta.VersionId;

            var resolvedReferencesAfterTransaction = fhirResponse.Resource.Entry[1].Resource.GetAllChildren<ResourceReference>().ToList()[0].Reference;

            Assert.True(resolvedReferencesAfterTransaction.Contains(idAfterTransaction, StringComparison.OrdinalIgnoreCase));
            Assert.True(resolvedReferencesAfterTransaction.Contains($"/_history/{versionIdAfterTransaction}", StringComparison.OrdinalIgnoreCase));
        }

        [Fact]
        [Trait(Traits.Priority, Priority.One)]
        public async Task GivenABundleWithVersionedReference_WhenSubmittingATransaction_ThenReferenceIsNotResolvedIfNotMatchedToFullUrl()
        {
            var requestBundle = Samples.GetJsonSample("Bundle-TransactionWithVersionSpecificReference").ToPoco<Bundle>();

            requestBundle.Entry[0].FullUrl = requestBundle.Entry[0].FullUrl.Split("/_history/1")[0];
            ...
            Assert.False(resolvedReferencesAfterTransaction.Contains(idAfterTransaction, StringComparison.OrdinalIgnoreCase));
            Assert.Equal(referenceBeforeTransaction, resolvedReferencesAfterTransaction);
        }
    }
}
