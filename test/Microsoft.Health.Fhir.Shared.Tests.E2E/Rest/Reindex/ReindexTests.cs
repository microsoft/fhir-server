...
namespace Microsoft.Health.Fhir.Tests.E2E.Rest.Reindex
{
    ...
    public class ReindexTests : IClassFixture<HttpIntegrationTestFixture>, IAsyncLifetime
    {
        ...
        private readonly IAsyncPolicy _retries;
        ...
        public async Task InitializeAsync()
        {
            ...
            {
                _output.WriteLine("ReindexTests.InitializeAsync: skipping reindex.");
            }
        ...
        public async Task GivenReindexJobWithMixedZeroAndNonZeroCountResources_WhenReindexCompletes_ThenSearchParametersShouldWork()
        {
            ...
            Assert.True(
                jobStatus.Status == OperationStatus.Completed,
                $"Expected Completed, got {jobStatus.Status}");
        ...
        public async Task GivenSearchParameterAddedAndReindexed_WhenSearchParameterIsDeleted_ThenAfterReindexSearchParameterShouldNotBeSupported()
        {
            ...
            Assert.True(
                jobStatus1.Status == OperationStatus.Completed,
                $"First reindex job should complete successfully, but got {jobStatus1.Status}");
            ...
            Assert.True(
                jobStatus2.Status == OperationStatus.Completed,
                $"Second reindex job should complete successfully, but got {jobStatus2.Status}");
        ...
        [InlineData(false, false)]
        [InlineData(false, true)]
        public async Task GivenSearchParamBundleCreate_ThenConflictWhenReindexAndSuccessOnNext(bool isBatch, bool isParallel)
        {
            if (!_isSql && !isBatch) // cosmos does not support transaction bundles
            {
                return;
            ...
            var bundle = new Bundle
            {
                ...
                [
                    new()
                    {
                        Resource = searchParams[0],
                        Request = new Bundle.RequestComponent { Method = Bundle.HTTPVerb.PUT, Url = $"SearchParameter/{codes[0]}" },
                    },
                    new()
                    {
                        Resource = searchParams[1],
                        Request = new Bundle.RequestComponent { Method = Bundle.HTTPVerb.PUT, Url = $"SearchParameter/{codes[1]}" },
                    },
                ],
            };
            var bundleOptions = new FhirBundleOptions { BundleProcessingLogic = isParallel ? FhirBundleProcessingLogic.Parallel : FhirBundleProcessingLogic.Sequential };
            ...
            // success - bundle create should succeed after reindex completes
            var bundleResponse = await _fixture.TestFhirClient.PostBundleAsync(bundle, bundleOptions);
            Assert.Equal(HttpStatusCode.OK, bundleResponse.Response.StatusCode);
            Assert.NotNull(bundleResponse.Resource?.Entry);
            Assert.Equal(2, bundleResponse.Resource.Entry.Count);
            ...
            {
                Assert.True(
                    entry.Response.Status == ((int)HttpStatusCode.Created).ToString() || entry.Response.Status == ((int)HttpStatusCode.OK).ToString(),
                    $"Unexpected status code '{entry.Response.Status}' for bundle create entry.");
            }
            ...
            foreach (var code in codes)
            {
                var resource = await _fixture.TestFhirClient.ReadAsync<SearchParameter>($"SearchParameter/{code}");
                Assert.NotNull(resource?.Resource);
                Assert.Equal(code, resource.Resource.Id);
            }
        }
        ...
        private async Task<int> DeleteResourcesAsync(string resourceType)
        {
            ...
            await _retries.ExecuteAsync(async () =>
            {
                ...
                do
                {
                    ...
                    var bundle = new Bundle { Type = _isSql ? Bundle.BundleType.Transaction : Bundle.BundleType.Batch };
                    foreach (var entry in searchResponse.Resource.Entry)
                    {
                        bundle.Entry.Add(new Bundle.EntryComponent { Request = new Bundle.RequestComponent { Method = Bundle.HTTPVerb.DELETE, Url = $"{resourceType}/{entry.Resource.Id}" } });
                    }
        ...
        private async Task<(OperationStatus Status, string FailureReason)> WaitForJobCompletionAsync(Uri jobUri, TimeSpan timeout)
        {
                                ...
                                Enum.TryParse<OperationStatus>(statusString, true, out var status))
                            {
                                    ...
                                    status == OperationStatus.Canceled)
                                {
                                    if (status == OperationStatus.Failed || status == OperationStatus.Canceled)
                                    {
                                        ...
                                        failureReason = failureParam?.Value is FhirString fs ? fs.Value : failureParam?.Value?.ToString();
            ...
            return (lastStatus, failureReason);
        }
