...
namespace Microsoft.Health.Fhir.Tests.E2E.Stu3
{
    public class FhirClient : ICustomFhirClient
    {
        ...
        {
            var json = EmbeddedResourceManager.GetStringContent("TestFiles", "Patient", "json", "Stu3");
            var parser = new Hl7.Fhir.Serialization.FhirJsonParser();
            return parser.Parse<Resource>(json).ToTypedElement().ToResourceElement();
        }
        ...
        {
            var json = EmbeddedResourceManager.GetStringContent("TestFiles", "Organization", "json", "Stu3");
            var parser = new Hl7.Fhir.Serialization.FhirJsonParser();
            return parser.Parse<Resource>(json).ToTypedElement().ToResourceElement();
        }
