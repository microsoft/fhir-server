...
namespace Microsoft.Health.Fhir.Tests.Integration.Features.ChangeFeed
{
    ...
    public class SqlServerFhirResourceChangeCaptureEnabledTests : IClassFixture<SqlServerFhirResourceChangeCaptureFixture>
    {
        ...
        public async Task GivenChangeCaptureEnabledAndNoVersionPolicy_AfterUpdating_InvisibleHistoryIsRemovedByWatchdog()
        {
            ...
            await store.SqlStoreClient.MergeResourcesAdvanceTransactionVisibilityAsync(CancellationToken.None); // this logic is invoked by WD normally
        ...
        public async Task GivenChangeCaptureEnabledAndNoVersionPolicy_AfterUpdating_HistoryIsNotReturnedAndChangesAreReturned()
        {
            ...
            await store.SqlStoreClient.MergeResourcesAdvanceTransactionVisibilityAsync(CancellationToken.None); // this logic is invoked by WD normally
            var startTranId = await store.SqlStoreClient.MergeResourcesGetTransactionVisibilityAsync(CancellationToken.None);
            ...
            await store.SqlStoreClient.MergeResourcesAdvanceTransactionVisibilityAsync(CancellationToken.None); // this logic is invoked by WD normally
            var endTranId = await store.SqlStoreClient.MergeResourcesGetTransactionVisibilityAsync(CancellationToken.None);
            ...
            // new style
            var trans = await store.SqlStoreClient.GetTransactionsAsync(startTranId, endTranId, CancellationToken.None);
            Assert.Equal(2, trans.Count);
            var resourceKeys = await store.SqlStoreClient.GetResourceDateKeysByTransactionIdAsync(trans[0].TransactionId, CancellationToken.None);
            ...
            resourceKeys = await store.SqlStoreClient.GetResourceDateKeysByTransactionIdAsync(trans[1].TransactionId, CancellationToken.None);
            Assert.Equal(1, resourceKeys.Count);
