...
namespace Microsoft.Health.Fhir.Tests.Integration.Features.Operations.Import
{
    ...
    public class SqlServerIndexesRebuildTests : IClassFixture<SqlServerFhirStorageTestsFixture>
    {
        ...
        private async Task<(SqlImportReindexer sqlImportOperation, SqlConnectionWrapperFactory sqlConnectionWrapperFactory, SqlServerFhirStorageTestHelper helper)> InitializeDatabaseAndOperation(string databaseName)
        {
            ...
            var sqlImportOperation = new SqlImportReindexer(sqlConnectionWrapperFactory, operationsConfiguration, schemaInformation, NullLogger<SqlImportReindexer>.Instance);
