...
END
        ...
        public async Task RollTransactionForward()
        {
            ...
            var tran = await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesBeginTransactionAsync(1, cts.Token, DateTime.UtcNow.AddHours(-1)); // register timed out
...
END
        ...
        public async Task AdvanceVisibility()
        {
            ...
            // create 3 trans
            var tran1 = await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesBeginTransactionAsync(1, cts.Token, DateTime.UtcNow.AddHours(1));
            var tran2 = await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesBeginTransactionAsync(1, cts.Token, DateTime.UtcNow.AddHours(1));
            var tran3 = await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesBeginTransactionAsync(1, cts.Token, DateTime.UtcNow.AddHours(1));
            var visibility = await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesGetTransactionVisibilityAsync(cts.Token);
            _testOutputHelper.WriteLine($"Visibility={visibility}");
            Assert.Equal(-1, visibility);

            // commit 1
            await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesCommitTransactionAsync(tran1.TransactionId, null, cts.Token);
            _testOutputHelper.WriteLine($"Tran1={tran1.TransactionId} committed.");

            startTime = DateTime.UtcNow;
            while ((visibility = await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesGetTransactionVisibilityAsync(cts.Token)) != tran1.TransactionId && (DateTime.UtcNow - startTime).TotalSeconds < 10)
            {
            ...
            // commit 3
            await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesCommitTransactionAsync(tran3.TransactionId, null, cts.Token);
            _testOutputHelper.WriteLine($"Tran3={tran3.TransactionId} committed.");

            startTime = DateTime.UtcNow;
            while ((visibility = await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesGetTransactionVisibilityAsync(cts.Token)) != tran2.TransactionId && (DateTime.UtcNow - startTime).TotalSeconds < 10)
            {
                await Task.Delay(TimeSpan.FromSeconds(0.1));
            }
            ...
            // commit 2
            await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesCommitTransactionAsync(tran2.TransactionId, null, cts.Token);
            _testOutputHelper.WriteLine($"Tran2={tran2.TransactionId} committed.");

            startTime = DateTime.UtcNow;
            while ((visibility = await _fixture.SqlServerFhirDataStore.SqlService.MergeResourcesGetTransactionVisibilityAsync(cts.Token)) != tran3.TransactionId && (DateTime.UtcNow - startTime).TotalSeconds < 10)
            {
                await Task.Delay(TimeSpan.FromSeconds(0.1));
            }
