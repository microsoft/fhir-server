...
Date `eq` on `dbo.DateTimeSearchParam` has two competing behaviors. Core emits spec-compliant **containment** — `SearchValueExpressionBuilderHelper` builds `DateTimeStart >= lo AND DateTimeEnd <= hi`, so the stored period must sit *inside* the query range (the FHIR R4 `eq` rule: https://hl7.org/fhir/R4/search.html#prefix). `DateTimeEqualityRewriter` (Core) then weakens that to **overlap** (`DateTimeStart <= hi AND DateTimeEnd >= lo`) for every date parameter.

To keep overlap index-friendly for the allow-listed `individual-birthdate`, ADR-2605 split an exact-day `eq` into a `UnionExpression` (SQL `UNION ALL`) on `IsLongerThanADay`. That union is the root cause of recurring SQL-generation failures in chained, `_sort`, and SMART/compartment shapes. Under containment a stored period longer than a one-day window can never be contained, so `IsLongerThanADay = 1` rows match nothing — the split, and the union, become unnecessary by construction.
...
Add **`EnableFhirDateContainment`** to `FhirSqlServerConfiguration` (standard config + `FHIRSERVER__…` env-var override) and pair it with the existing **`EnableScalarTemporalEqualityRewriter`**. The temporal `UNION ALL` is removed regardless of flag values.
...
The rewriter runs **only when both flags are on**, emitting a single `IsLongerThanADay = 0 AND DateTimeEnd = endOfDay` predicate (ADR-2605's index optimization) — never a `UnionExpression`. Overlap weakening is skipped iff containment is on, so Core's containment flows to SQL for every date parameter.
...
- Cosmos stays on overlap, so SQL and Cosmos can diverge when the flag is on (tracked follow-up).
- Supersedes ADR-2605: its union optimization is replaced by the union-free containment model it anticipated. Tracked alongside AB#191826.
