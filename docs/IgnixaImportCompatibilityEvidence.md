...
| --- | --- | --- |
| `ImportResourceParserParityTests` | Real `FirelyImportResourceParser` and `IgnixaImportResourceParser`, each with the real `ResourceWrapperFactory` and `RawResourceFactory` | Both parsers produce the documented import metadata and raw-resource outputs for valid inputs, or each returns an error for invalid inputs. |
...
| Duplicate known property and unknown property | Explicit output and error assertions | Firely retains the final duplicate `active` value, while Ignixa rejects the duplicate with `ArgumentException`. An unknown member is rejected by both paths rather than being silently persisted. |
